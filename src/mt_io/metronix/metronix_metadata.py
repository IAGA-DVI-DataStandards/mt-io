...
class MetronixRunXML:
    ...
    ATSWRITER_CONFIG_KEY = (
        "./recording/output/ProcessingTree/output/ATSWriter/configuration"
    )
    ...
    def __str__(self) -> str:
        ...
        for ch_id, ch_data in self._channels.items():
            lines.append(
                f"    [{ch_id}] {ch_data.get('channel_type', '?')}: {ch_data.get('ats_data_file', '?')}"
            )
        return "\n".join(lines)
    ...
    def _parse_channel_outputs(self) -> None:
        ...
        if not channel_outputs:
            ...
            for pt_path in [
                "./recording/output/ProcessingTree",
        ...
        for channel_elem in channel_outputs:
            ...
            ch_data = {
                ...
                "sample_rate": float(
                    channel_elem.findtext("sample_freq", str(self.sample_rate))
                ),
        ...
        if channel_id in self._calibrations:
            ...
            for cal_entry in self._calibrations[channel_id]:
                if "filter_name" in cal_entry:
                    metadata_obj.add_filter(
                        AppliedFilter(
                            name=cal_entry["filter_name"], applied=True, stage=stage
                        )
                    )
                    stage += 1
