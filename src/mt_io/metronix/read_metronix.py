...
def read_metronix(file_path: str | pathlib.Path) -> ATSS:
    ...
    if file_path.suffix.lower() not in [".atss", ".ats"]:
        raise ValueError(
            f"File {file_path} does not have a valid Metronix extension (.atss or .ats)"
        )

    if file_path.suffix.lower() == ".ats":
        logger.warning(
            f"File {file_path} has .ats extension, but .atss is expected. Attempting to read as ATSS."
        )
        raise NotImplementedError(
            "Reading .ats files is not implemented yet. Please convert to .atss format."
        )
